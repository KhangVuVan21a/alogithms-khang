...
public class Bai16maxByPrice {
	public static void main(String[] args) {
		...
		System.out.println(maxByPrice(listProducts));
		
	}
	public static Product maxByPrice(ArrayList<Product> listProducts) {
		Product maxProduct= listProducts.get(0);
		for(int i=1;i<listProducts.size();i++) {
			if(maxProduct.getPrice()<listProducts.get(i).getPrice()) {
				maxProduct=listProducts.get(i);
			}
		}
		return maxProduct;
	}
